...
It also has tools to generate a final report in both Markdown and [Threat Composer](https://github.com/awslabs/threat-composer`) JSON format (.tc.json).
...
- Exported threat models (Threat Composer JSON format (.tc.json))
- Validation reports
- Analysis results
...
2. Install all dependencies (including dev/test): `uv sync`
3. Run the server locally: `uv run run_server.py`
